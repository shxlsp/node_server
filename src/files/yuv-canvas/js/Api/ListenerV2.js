...
g_mapUserIdToStream = new Map();
g_curSelView = undefined;

/**
 * ListenerV2 Module:Used to receive event callbacks(该类用于接收事件回调)
 * @module ListenerV2
 */
class ListenerV2{
    /**
     ...
     */
    onFirstVideoFrame (userId,streamId){
        console.log("onFirstVideoFrame,userId:" + userId + ",streamId:" + streamId);
    };
     ...
     * @param {Object[]} userList An array list of users in a meeting(会议内用户信息数组列表)
     * @param {string} userList[].userId The ID of the participant in the meeting(会议中的参会者ID)
     * @param {string} userList[].nickName The user nickname(参会者的用户昵称)
     */
    onEnterRoom(userList){
        console.log("enter onEnterRoom");
        var size = userList.length;
        console.log("onEnterRoom userList size:" + size);
        for (var i=0;i<size;i++) {
            console.log("onEnterRoom nickName:" +  userList[i].nickName + ",userId:" + userList[i].userId);
        }
    }
     ...
     */
    onRemoteUserEnterRoom(userId,nickname){
        console.log("onRemoteUserEnterRoom，userId:" + userId + ",nickname:" + nickname);
    }
     ...
     */
    onUserDesktopAvailable(userId,streamId,streamName,bAvailable,nickname){
        console.log("onUserDesktopAvailable,userId:" + userId + ",streamId:" + streamId + ",streamName:" + streamName + ",bAvailable:" + bAvailable + ",nickname:" + nickname);
        ...
        if(bAvailable){
            ...
            remoteView.setAttribute('extraDataUserId',userId);
            ...
            listenerV2.jrtcengine.startRemoteDesktopView(remotePreviewParam);
        }else{
            //17.关闭屏幕共享流
            var remoteView = document.getElementById(streamId);
            console.log("before stopRemoteDesktopView:",listenerV2);
            listenerV2.jrtcengine.stopRemoteDesktopViewV2(userId,streamId,remoteView);
            console.log("after stopRemoteDesktopView:",listenerV2);
     ...
     * @param {string} userId The ID of the participant in the meeting(会议中的参会者ID)
     */
    onRemoteUserLeaveRoom(reason,userId){
        console.log("onRemoteUserLeaveRoom,userId:" + userId + ",reason:" + reason);
    }
     ...
     * @param {string} nickname the nickname of user(用户昵称)
     */
    onUserVideoAvailable(userId,streamId,streamName,bAvailable,nickname){
        console.log("onUserVideoAvailable,userId:" + userId + ",streamId:" + streamId + ",streamName:" + streamName + ",bAvailable:" + bAvailable + ",nickname:" + nickname);
        //6.start remote video
        var parentNode = document.getElementById('remote');
        if(bAvailable){
            //设置userId 和 streamId
            if(streamName != "false"){
                g_mapUserIdToStream.set(userId,streamId);
            }
            var remoteView = document.createElement('div');
            remoteView.setAttribute('id',streamId);
            remoteView.setAttribute('extraDataUserId',userId);
     ...
     * @param {string} nickname the nickname of user(用户昵称)
     */
    onUserAudioAvailable(userId,streamId,bAvailable,customStreamType,nickname){
        console.log("onUserAudioAvailable,userId:" + userId + ",streamId:" + streamId + ",bAvailable:" + bAvailable + ",customStreamType:" + customStreamType + ",nickname:" + nickname);       
    }
     ...
     * @param {string} nickName The user nickname(参会者的用户昵称)
     */
    onUserUpdateNickName(userId,nickName){
        console.log("onUserUpdateNickName，userId："+ userId + ",nickname:" + nickName);
    }

    /**
     * @desc 【desc】This function is called when the user volume changes(用户音量大小改变时调用此函数)
     * @param {string} volume 音量信息,取值范围是(-127到0),值越大音量越大.格式为：
     * {
     *   "roomId":"173",
     *   "volumeInfos":[
     *      {
     ...
     */
    onUserMessageReceived(userId,nickName,msg){
        console.log("onUserMessageReceived,userId:" + userId + ",nickname:" + nickName + ",msg:" + msg);
    }
     ...
     */
    onUserRemoved(userId,nickName,appData){
        console.log("onUserRemoved,userId:" + userId + ",nickname:" + nickName + ",appData:" + appData);
    }
     ...
     * @param {string} appData additional data(附加数据)
     */
    onAudioMuteToRoom(userId,nickName,appData){
        console.log("onAudioMuteToRoom,userId:" + userId + ",nickname:" + nickName + ",appData:" + appData);
    }
     ...
     * @param {string} appData additional data(附加数据)
     */
    onAudioMuteToPeer(userId,nickName,appData){
        console.log("onAudioMuteToPeer,userId:" + userId + ",nickname:" + nickName + ",appData:" + appData);
    }
     ...
     * @param {string} appData additional data(附加数据)
     */
     onVideoCloseToRoom(userId,nickName,appData){
        console.log("onVideoCloseToRoom,userId:" + userId + ",nickname:" + nickName + ",appData:" + appData);
     }
     ...
     * @param {string} appData additional data(附加数据)
     */
     onVideoCloseToPeer(userId,nickName,appData){
        console.log("onVideoCloseToPeer,userId:" + userId + ",nickname:" + nickName + ",appData:" + appData);
     }
     ...
     * @param {string} appData additional data(附加数据)
     */
    onForbiddenChatToRoom(userId,nickName,appData){
        console.log("onForbiddenChatToRoom,userId:" + userId + ",nickname:" + nickName + ",appData:" + appData);
    }
     ...
     * @param {string} appData additional data(附加数据)
     */
    onUnForbiddenChatToRoom(userId,nickName,appData){
        console.log("onUnForbiddenChatToRoom,userId:" + userId + ",nickname:" + nickName + ",appData:" + appData);
    }
    ...
    /**
     * @desc 【desc】get g_mapUserIdToStream 
     */
    getMapUserIdToStream () {
        return g_mapUserIdToStream;
    }

    /**
     * @desc 【desc】 get currrent selected view
     */
    getCurSelView(){
        return g_curSelView;
    }
    ...
    /**
     * @desc 【desc】 onAudioHowlingDetected
     */
    onAudioHowlingDetected(isDetected){
        console.log("onAudioHowlingDetected,isDetected:" + isDetected);
    }

    /**
     * @desc 【desc】 onEventReportCallback
     */
    onEventReportCallback(info){
        console.log("onEventReportCallback,info:" + info);
    }
     ...
     */
     onUpdateCustomStreamType(userId,kind,streamId,customStreamType){
        console.log("onUpdateCustomStreamType,peerId:" + userId + ",kind:" + kind + ",streamId:" + streamId + ",customStreamType:" + customStreamType);
    }
}
...
function onRemoteClickedEvent () {
    ...
    var userId = this.getAttribute('extraDataUserId');
    var divUserId = document.getElementById('userId');
    divUserId.value = userId;
    var divStreamId = document.getElementById('streamId');
    divStreamId.value = streamID;
    g_curSelView = this;
};

module.exports = ListenerV2;